...
[CreateAssetMenu(fileName = "StatusOnCrit", menuName = "RPG/SkillEffect/StatusOnCrit")]
public class StatusOnCrit : SkillEffect
{
    ...
    public override void Apply(Combatant user, Combatant target, Skill skill, SkillContext skillctx)
    {
            ...
            if (ctx.cancelled)
        {
            ...
            if(!skillctx.lastHealCrit)
        {
            Debug.Log($"{user.character.characterName} did not crit, {newStatus.Name} was not applied!");
            return;
        }
